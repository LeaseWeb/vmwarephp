...
class PropertyFilterSpecTest extends PHPUnit_Framework_TestCase
{
    ...
    function testMakesAPropertyFilterSpecForGatheringAListOrPropertiesForAManagedObject()
    {
        ...
        $this->assertEquals(
            $expectedPropertyFilterSpec,
            $this->factory->makeForOneManagedObject($managedObjectType, $referenceId, $requestedProperties)
        );
    }
    ...
    function testMakesAPropertyFilterSpecForGatheringTraversalPropertiesForAManagedObject()
    {
        ...
        $propSet = array(
            new \PropertySpec('Network', false, array('configStatus')),
            new \PropertySpec($managedObjectType, false, array('configStatus'))
        );
        $objectSet = array(
            new \ObjectSpec(
    ...
    function testMakesAPropertyFilterSpecForTraversingAllInventoryForAGivenManagedObjectTypeAndItsProperties()
    {
        ...
        $this->assertEquals(
            $expectedPropertyFilterSpec,
            $this->factory->makeForTraversingAllInventory($managedObjectType, $requestedProperties, $vmwareService)
        );
    }
    ...
    private function mockContainerViewUsingService($vmwareService, $managedObjectType)
    {
        ...
        $aContainerView = $this->aManagedObjectThatExpectsToBeCastToReference(
            new ManagedObjectReference('1', 'ContainerView')
        );
        $aContainerManager->shouldReceive('CreateContainerView')->with(
            array(
